...
TEST_F(VkLayerTest, TestSurfaceQueryImageCompressionControlWithoutExtension) {
    ...
    image_compression_control.imageCompressionControl = false;
    ...
    // get compression control properties even of VK_EXT_image_compression_control extension is disable(or is not supported).
    m_errorMonitor->SetDesiredFailureMsg(kErrorBit, "VUID- VK_EXT_image_compression_control should not exist");
    vkGetPhysicalDeviceSurfaceFormats2KHR(gpu(), &surface_info, &count, nullptr);
    m_errorMonitor->VerifyFound();
}